...
public class tab2 extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        mrecyclerView = v.findViewById(R.id.rvTab2);
        mrecyclerView.setLayoutManager(new LinearLayoutManager(v.getContext()));
        mSwipeRefreshLayout = v.findViewById(R.id.srlTab2);
        ...
        FirebaseRecyclerOptions<Question> options =
                new FirebaseRecyclerOptions.Builder<Question>()
                        .setQuery(FirebaseDatabase.getInstance().getReference().child("Question").child(UserDetails.username).orderByChild("status").equalTo("pending"), Question.class)
                        .build();
...
//
//        }
        adapter = new QuestionAdapter2(getActivity(), options);
